      ...
      {total > 0 && (
        <button
          className={`w-full py-3 rounded-lg font-medium ${(!user || paymentProcess)
            ? "bg-gradient-to-r from-slate-300 to-gray-300 hover:cursor-not-allowed"
...
const CartWithItems= ({ items, user, paymentProcess,setPaymentProcess}: CartWithItemsProps) => {
  const { total } = useCart();
  return (
    <section className="container mx-auto px-1 py-2 fade">
...
const CartSection = ({ user }) => {
  ...
  return (
    <section className="container mx-auto px-1 py-2 animate-fade-in">
